...
import { fetchProjectFilterOptions, saveProjectFilterOption, deleteProjectFilterOption } from '@/lib/api';
...
const GROUPS: { key: ProjectFilterOption['group']; zh: string; en: string }[] = [
  { key: 'scene', zh: '应用场景', en: 'Scene' },
  { key: 'maturity', zh: '成熟度', en: 'Maturity' },
  { key: 'category', zh: '类别', en: 'Category' },
];

const emptyOption = (): Partial<ProjectFilterOption> => ({ group: 'scene', name: '', name_en: '', sort_order: 0, is_active: true, is_featured: false });

export default function AdminProjectFilters() {
  const { t, lang } = useI18n();
  const { profile } = useAuth();

  const [items, setItems] = useState<ProjectFilterOption[]>([]);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState<Partial<ProjectFilterOption> | null>(null);

  const load = useCallback(() => {
    fetchProjectFilterOptions().then(setItems).catch(() => toast.error(t('加载失败', 'Load failed'))).finally(() => setLoading(false));
  }, [t]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      await saveProjectFilterOption(editing);
  ...
  const handleDelete = async (id: string) => {
    try { await deleteProjectFilterOption(id); toast.success(t('已删除', 'Deleted')); load(); }
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <p className="editorial-label">{t('项目筛选配置', 'Project Filter Config')}</p>
          <h2 className="mt-1 font-display text-xl font-medium">{t('自定义项目库筛选项', 'Customize project filters')}</h2>
        </div>
        ...
        <div className="mt-6 space-y-4 border border-border bg-card p-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field label={t('分组', 'Group')}>
              <Select value={editing.group || 'scene'} onValueChange={(v) => setEditing({ ...editing, group: v as ProjectFilterOption['group'] })}>
          ...
          <div className="flex items-center gap-3">
            <Switch id="p-active" checked={!!editing.is_active} onCheckedChange={(v) => setEditing({ ...editing, is_active: v })} />
            <Label htmlFor="p-active" className="font-mono-label text-xs uppercase tracking-wider text-muted-foreground">{t('启用', 'Active')}</Label>
          </div>
          <div className="flex items-center gap-3">
            <Switch id="p-featured" checked={!!editing.is_featured} onCheckedChange={(v) => setEditing({ ...editing, is_featured: v })} />
            <Label htmlFor="p-featured" className="font-mono-label text-xs uppercase tracking-wider text-muted-foreground">{t('默认首页展示', 'Show on homepage')}</Label>
          </div>
                ...
                <table className="w-full min-w-[600px] border-collapse">
                  <thead>
                    <tr className="border-b border-border bg-muted/40">
                      ...
                      <th className="whitespace-nowrap px-4 py-3 text-left font-mono-label text-xs uppercase tracking-wider text-muted-foreground">{t('首页展示', 'Featured')}</th>
                  ...
                  <tbody>
                    {g.items.length === 0 ? (
                      ...
                      <tr key={item.id} className="border-b border-border last:border-0">
                        ...
                        <td className="whitespace-nowrap px-4 py-3 text-sm text-muted-foreground">{item.is_featured ? t('是', 'Yes') : t('否', 'No')}</td>
