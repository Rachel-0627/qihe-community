...
import { fetchPromptCaseFilters, savePromptCaseFilter, deletePromptCaseFilter } from '@/lib/api';
...
import type { PromptCaseFilter, PromptCaseFilterGroup } from '@/types/types';

const GROUPS: PromptCaseFilterGroup[] = ['category', 'style', 'scene'];

const groupLabel = (group: PromptCaseFilterGroup, t: (zh: string, en: string) => string) => {
  switch (group) {
    case 'category': return t('分类', 'Category');
    case 'style': return t('风格', 'Style');
    case 'scene': return t('场景', 'Scene');
    default: return group;
  }
};

const emptyFilter = (): Partial<PromptCaseFilter> => ({ group: 'category', name: '', name_en: '', sort_order: 0, is_active: true });

export default function AdminPromptCaseFilters() {
  const { t, lang } = useI18n();
  const { profile } = useAuth();

  const [items, setItems] = useState<PromptCaseFilter[]>([]);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState<Partial<PromptCaseFilter> | null>(null);

  const load = useCallback(() => {
    fetchPromptCaseFilters()
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!editing.group) { toast.error(t('请选择维度', 'Group is required')); return; }
    try {
      await savePromptCaseFilter(editing);
  ...
  const handleDelete = async (id: string) => {
    try { await deletePromptCaseFilter(id); toast.success(t('已删除', 'Deleted')); load(); }
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <p className="editorial-label">{t('提示词案例筛选配置', 'Prompt Case Filter Config')}</p>
          <h2 className="mt-1 font-display text-xl font-medium">{t('自定义分类 / 风格 / 场景', 'Customize categories, styles & scenes')}</h2>
        </div>
        <Button onClick={() => setEditing(emptyFilter())} className="gap-1.5 font-mono-label text-xs"><Plus className="h-3.5 w-3.5" />{t('新增筛选项', 'Add filter')}</Button>
      </div>

      {editing && (
        <div className="mt-6 space-y-4 border border-border bg-card p-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field label={t('维度', 'Group')}>
              <Select value={editing.group} onValueChange={(v) => setEditing({ ...editing, group: v as PromptCaseFilterGroup })}>
                <SelectTrigger className="px-3"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {GROUPS.map((g) => <SelectItem key={g} value={g}>{groupLabel(g, t)}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
        ...
        <table className="w-full min-w-[600px] border-collapse">
          <thead>
            <tr className="border-b border-border bg-muted/40">
              <th className="whitespace-nowrap px-4 py-3 text-left font-mono-label text-xs uppercase tracking-wider text-muted-foreground">{t('维度', 'Group')}</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-6 text-center text-sm text-muted-foreground">{t('暂无筛选项', 'No filters')}</td></tr>
            ) : items.map((item) => (
              <tr key={item.id} className="border-b border-border last:border-0">
                <td className="whitespace-nowrap px-4 py-3 text-sm text-foreground">{groupLabel(item.group, t)}</td>
